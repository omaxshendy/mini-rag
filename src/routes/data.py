...
async def process_endpoint(request: Request,project_id:int,process_request:ProcessRequest):
    ...
    else: # (process_request.file_id == None)
        # asset_model= await AssetModel.create_instance(db_client=request.app.db_client)
    ...
    # prior to the for-loop of chunking
    no_records=0
    no_files=0
...
async def upload_data(request:Request,project_id:int,file:UploadFile,app_settings:Settings=Depends(get_settings)):
    ...
    file_path,file_id=data_controller.generate_unique_filepath(original_filename=file.filename,
